...
#plot
hex <- hue_pal()(12)
p <- ggplot() + geom_point(data=all_stn%>%drop_na(Q_m3s_02,CO2_ppm_01),aes(x=Q_m3s_02*1000,y=CO2_ppm_01),color="grey") +
  geom_point(data=all_storm,aes(x=Q_m3s*1000,y=pCO2_ppm,color=month)) +
  scale_color_manual(values=c(hex[4],hex[5],hex[6],hex[7],hex[8],hex[9],hex[10],hex[11],hex[12])) +
  scale_y_continuous(transform = "log", breaks = c(2000,6000,16000))+ 
  scale_x_continuous(transform = "log", breaks = c(3,10,30,100))+ ylab(expression(italic(p)~CO[2] ~'(ppm)')) +    
