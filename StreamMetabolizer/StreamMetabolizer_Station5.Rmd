...
stn05 <- read.csv(here::here("metabolizer_dataframe/stn05_df_march09.csv"))
stn05$solar.time <- as.POSIXct(stn05$solar.time,format="%Y-%m-%d %H:%M:%S",tz="UTC")

#let's see about just 2019 data
dat <- stn05#%>%
#  filter(solar.time < as.POSIXct("2021-06-27 01:02:13"))#&
#  filter(solar.time > as.POSIXct("2021-06-27 01:02:13")))#&
...
#since we don't much about GPP in this stream, remove GPP prior
bayes_specs <- specs(bayes_name, burnin_steps=1000, saved_steps=1000, n_cores=1)
...
#for stn6 it is: 0.06357686 to 0.7613477
#take the ln() and enter in as prior

#bayes_specs$K600_lnQ_nodes_centers<-seq(-3, -.3, 0.5)

#do below later? maybe I don't need to??

###CHANGE DEPTH AND SLOPE FOR EACH SITE
bayes_specs$K600_lnQ_nodes_meanlog <- rep(prior_K600_lnQ_nodes_meanlog(slope=0.0326, depth=0.299534),length(bayes_specs$K600_lnQ_nodes_centers))

bayes_specs$K600_lnQ_nodes_sdlog <- rep(0.7, length(bayes_specs$K600_lnQ_nodes_centers))
...
#save model
#save(mm, file = "model_stn05_partially-pooled_stn05_feb19_all.RData")

predictions <- predict_metab(mm)
#write.csv(predictions,here::here("StreamMetabolizer/Predictions/MetabPredictions_stn04_all_feb11df.csv"))
...
daily_fit <- get_fit(mm)$daily

write.csv(daily_fit,here::here("StreamMetabolizer/Predictions_bestones/station5_calclight_march9.csv"))
...
ggplot(data_inspection,aes(x=discharge,y=K600.daily)) + geom_point()
...
#write.csv(data_inspection,here::here("StreamMetabolizer/Predictions/data_inspection_stn05_feb19_all.csv"))
