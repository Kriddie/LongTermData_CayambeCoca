...
DO_01 <- DO_01%>%filter(DO_mgL > -100)%>%
  filter(DateTime<as.POSIXct("2021-06-10 00:00:00",tz="UTC")|
           DateTime>as.POSIXct("2021-06-10 11:15:00",tz="UTC"))%>%
   filter(DateTime!=as.POSIXct("2021-06-14 11:00:00",tz="UTC"))

DO_01$DOTemp_c <- ifelse(DO_01$DOTemp_c > 30, (DO_01$DOTemp_c-23)/9*5,DO_01$DOTemp_c)
##check data
p1 <- plot_ly(DO_01, x = ~DateTime, y = ~DO_mgL, type = 'scatter', mode = 'markers') 

p2 <- plot_ly(DO_01, x = ~DateTime, y = ~DOTemp_c, type = 'scatter', mode = 'markers') 
