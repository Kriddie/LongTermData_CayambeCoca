...
Baro$X <- NULL
Baro <- unique(Baro)
Baro <- Baro%>%filter(DateTime!=as.POSIXct("2021-06-18 11:00:00", tz="UTC"))%>%filter(DateTime!=as.POSIXct("2022-07-12 10:15:00
",tz="UTC"))%>%filter(DateTime!=as.POSIXct("2023-02-18 12:30:00
",tz="UTC"))

#test for dubplicates
test <- WL_01$DateTime[duplicated(WL_01$DateTime)]
...
WLData <- read.csv(here::here("WL/lvlgr_2020421_2019-08-14.csv"), skip=11, header = TRUE, sep = ",",quote = "\"",dec = ".", fill = TRUE, comment.char = "")
...
WL_03 <- unique(WL_03)
WL_03 <- WL_03%>%drop_na(WLPres_kpa)


#test for duplicates
test <- WL_03$DateTime[duplicated(WL_03$DateTime)]
test2 <- as.data.frame(test)
...
mtext("y= 2830.62451  - 86.93018 *x + 0.68280 *x^2", side=3)
...
WL_01_fill <- WL_03
WL_01_fill$WLPres_kpa <-   2830.62451 - 86.93018 *WL_01_fill$WLPres_kpa + 0.68280 *(WL_01_fill$WLPres_kpa)^2
#WL_01_fill$WLPres_kpa <-   485.6142 - 13.92432 *WL_01_fill$WLPres_kpa + 0.1144849 *(WL_01_fill$WLPres_kpa)^2
WL_01_fill <- WL_01_fill%>%filter(DateTime > as.POSIXct("2019-07-23 11:45:00", tz = "UTC")& DateTime < as.POSIXct("2019-07-25 21:45:00", tz = "UTC"))
WL_01_fill$Station <- "WL_01"
WL_01 <- WL_01%>%filter(DateTime <= as.POSIXct("2019-07-23 11:45:00", tz = "UTC")| DateTime >= as.POSIXct("2019-07-25 21:45:00", tz = "UTC"))
...
setwd(here::here("CO2"))
all_files=list.files(pattern=".csv") #pulls out the csv files from WL 
...
#test for dubplicates
test <- CO2_01$DateTime[duplicated(CO2_01$DateTime)]
test2 <- as.data.frame(test)

#filter <- CO2_01%>%filter(DateTime>as.POSIXct("2021-07-28 00:00:00",tz='UTC'))
```
# 2019 data
...
#test for duplicates
test <- Stn01_newV$DateTime[duplicated(Stn01_newV$DateTime)]
test2 <- as.data.frame(test)
...
co2_1 <- ggplot(data = Stn01_oldV, aes(DateTime, CO2_ppm_adjusted)) + geom_point(color = "steelblue") +
  labs(y = "CO2 ppm", x = "") 
...
#write.csv(Stn01_oldV, here::here("data_cleaned/CO2_01_max10000_cleaned.csv"),row.names = FALSE)
...
#    }
  ...
  DOData$DateTime <- round_date(DOData$DateTime, "5 mins")
  DOData$Station <- site
...
#clean
DO_01 <- DO_01%>%filter(DO_mgL > -888.88)

#test for duplicates
test <- DO_01$DateTime[duplicated(DO_01$DateTime)]
test2 <- as.data.frame(test)
...
#write.csv(DO_01, here::here("data_cleaned/DO_01_cleaned_DataExporation.csv"),row.names = FALSE)
