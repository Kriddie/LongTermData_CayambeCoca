...
```
## plot 
plot the kpa water level so that each station data can be cleaned

```{r plot, echo=FALSE}

p1 <- plot_ly(WL_04, x = ~DateTime, y = ~WLPres_kpa, type = 'scatter', mode = 'markers') 
...
WL_04[WL_04$DateTime < as.POSIXct("2021-07-18 02:45",tz="UTC"),]$Q_m3s <- 0.8086*(WL_04[WL_04$DateTime < as.POSIXct("2021-07-18 02:45",tz="UTC"),]$WL_m)^2 + 0.3622*(WL_04[WL_04$DateTime < as.POSIXct("2021-07-18 02:45",tz="UTC"),]$WL_m) - .0042

WL_04[WL_04$DateTime > as.POSIXct("2021-07-18 06:00",tz="UTC"),]$Q_m3s <- 3.3124*(WL_04[WL_04$DateTime > as.POSIXct("2021-07-18 06:00",tz="UTC"),]$WL_m)^2 - 0.4161*(WL_04[WL_04$DateTime > as.POSIXct("2021-07-18 06:00",tz="UTC"),]$WL_m) + 0.0091

#also, delete the data between the change in rating curves
WL_04 <- WL_04%>%drop_na(Q_m3s)
#may need to revisit this, but perhapes it is fair to say that at certain periods the flow is basically 0. I'm going to make anything less than 0 Q, 0)

WL_04[WL_04$Q_m3s < 0,]$Q_m3s <- 0
...
Stn04_df <- full_join(WL_04,CO2_04, by=c("DateTime","Station"))
...
#bind lux data
colnames(LUXabajo_04_Lux_data) <- c("DateTime","LUXabajoTemp_c","LUXabajo","Station")
colnames(LUXarriba_04_Lux_data) <- c("DateTime","LUXarribaTemp_c","LUXarriba","Station")

Lux_04 <- full_join(LUXarriba_04_Lux_data, LUXabajo_04_Lux_data, by=c("DateTime","Station"))
...
Stn04_df <- full_join(Stn04_df, DO_04, by=c("DateTime","Station"))
Stn04_df <- full_join(Stn04_df, Lux_04, by=c("DateTime","Station"))

```

```{r write out data, echo=FALSE}
#write.csv(Stn04_df,here::here("MergedFiles/Stn04_df_2023-12-20.csv"))
