...
WL_02_sub <- WL_02%>%filter(DateTime>=as.POSIXct("2022-06-30 12:45:00",tz='UTC')&
                            DateTime<=as.POSIXct("2022-06-30 13:30:00",tz='UTC'))
WL_02_sub <- WL_02_sub%>%group_by(DateTime,Station)%>%
  summarise(WLPres_kpa=mean(WLPres_kpa),
            WLTemp_c=mean(WLTemp_c))
WL_02 <- WL_02%>%filter(DateTime<as.POSIXct("2022-06-30 12:45:00",tz='UTC')|
                            DateTime>as.POSIXct("2022-06-30 13:30:00",tz='UTC'))

WL_02 <- rbind(WL_02,WL_02_sub)
...
#test for duplicates
test <- Baro$DateTime[duplicated(Baro$DateTime)]
...
CO2_02 <- unique(CO2_02)
CO2_02 <- CO2_02%>%filter(DateTime<as.POSIXct("2023-02-19 01:00:00",tz='UTC'))

#test for duplicates
test <- CO2_02$DateTime[duplicated(CO2_02$DateTime)]
test2 <- as.data.frame(test)
...
#write.csv(Stn02, here::here("data_cleaned/CO2_02_max10000_cleaned_may14.csv"),row.names = FALSE)
