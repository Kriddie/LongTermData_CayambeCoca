...
stn03 <- full_join(WL_df,CO2_df,by=c("DateTime","Station"))%>%select(DateTime,Q_m3s,CO2_ppm_adjusted,WLTemp_c)

stn03 <- stn03%>%filter(DateTime > as.POSIXct("2021-03-15 03:30:00"))

stn03 <- unique(stn03)

test <- stn03$DateTime[duplicated(stn03$DateTime)]
...
CO2_df <- CO2_df%>%filter(
  DateTime!=as.POSIXct("2021-06-18 11:00:00",tz="UTC"))%>%filter(
  DateTime!=as.POSIXct("2023-02-18 10:45:00",tz="UTC"))%>%filter(DateTime<as.POSIXct("2022-06-16 15:45:00",tz="UTC")|DateTime>as.POSIXct("2022-06-16 16:35:00",tz="UTC"))


stn04 <- full_join(WL_df,CO2_df,by=c("DateTime","Station"))

stn04 <- stn04%>%filter(DateTime!=as.POSIXct("2021-06-18 11:00:00",tz="UTC"))%>%filter(DateTime<as.POSIXct("2022-06-16 15:45:00",tz="UTC")|DateTime>as.POSIXct("2022-06-16 16:30:00",tz="UTC"))%>%filter(DateTime!=as.POSIXct("2022-07-12 10:15:00",tz="UTC"))%>%filter(DateTime!=as.POSIXct("2023-02-18 10:45:00",tz="UTC"))

#test for duplicates
test <- stn04$DateTime[duplicated(stn04$DateTime)]
test2 <- as.data.frame(test)
...
stn05 <- stn05%>%filter(DateTime!=as.POSIXct("2022-07-12 10:15:00",tz="UTC"))%>%filter(DateTime!=as.POSIXct("2021-06-18 11:00:00",tz="UTC"))%>%filter(DateTime!=as.POSIXct("2023-02-18 12:30:00",tz="UTC"))
#test for duplicates
...
stn06 <- stn06%>%filter(DateTime!=as.POSIXct("2022-07-12 10:15:00",tz="UTC"))%>%filter(DateTime!=as.POSIXct("2021-06-18 11:00:00",tz="UTC"))%>%filter(DateTime!=as.POSIXct("2023-02-18 12:30:00",tz="UTC"))
#test for duplicates
...
### join all
all_stn <- full_join(stn01 %>%select(DateTime,Q_m3s,CO2_ppm_adjusted,WLTemp_c)%>%rename(Q_m3s_01 = Q_m3s,CO2_ppm_01=CO2_ppm_adjusted,WaterTemp_c_01=WLTemp_c),
                     stn02 %>%select(DateTime,Q_m3s,CO2_ppm_adjusted,WLTemp_c)%>%rename(Q_m3s_02 = Q_m3s,CO2_ppm_02=CO2_ppm_adjusted,WaterTemp_c_02=WLTemp_c),
                     by="DateTime")

test <- all_stn$DateTime[duplicated(all_stn$DateTime)]
test2 <- as.data.frame(test)

all_stn <- full_join(all_stn,
                     stn03 %>%select(DateTime,Q_m3s,CO2_ppm_adjusted,WLTemp_c)%>%rename(Q_m3s_03 = Q_m3s,CO2_ppm_03=CO2_ppm_adjusted,WaterTemp_c_03=WLTemp_c),
                     by="DateTime")

test <- all_stn$DateTime[duplicated(all_stn$DateTime)]
test2 <- as.data.frame(test)

all_stn <- full_join(all_stn,stn04 %>%
                       select(DateTime,Q_m3s,CO2_ppm_adjusted,WLTemp_c)%>%
                       rename(Q_m3s_04 = Q_m3s,CO2_ppm_04=CO2_ppm_adjusted,WaterTemp_c_04=WLTemp_c),
                     by="DateTime")
...
#average pco2 from june- aug
stn01_wetseason <- stn01
stn01_wetseason$month <- format(as.Date(stn01_wetseason$DateTime), "%B")
stn01_wetseason$season <- ifelse(stn01_wetseason$month=="June"|stn01_wetseason$month=="July"|stn01_wetseason$month=="August","wet",
                                 ifelse(stn01_wetseason$month=="November"|stn01_wetseason$month=="December"|stn01_wetseason$month=="January"|stn01_wetseason$month=="February","dry",NA))

stn01_wetseason_summ <- stn01_wetseason %>%group_by(season)%>%
  summarise(mean_co2 = mean(CO2_ppm_adjusted,na.rm = TRUE),
            sd= sd(CO2_ppm_adjusted,na.rm = TRUE))
...
#stn 1 146 out of 401
do_stn1 <- read.csv(here::here("data_cleaned/DO_01_cleaned_DataExporation.csv"))
do_stn1$DateTime <- as.POSIXct(do_stn1$DateTime,format="%Y-%m-%d %H:%M",tz="UTC")
do_stn1$Date <- as.Date(do_stn1$DateTime)

do_stn1_anoxic <- do_stn1%>%filter(DO_mgL<=0)
do_stn1_hypoxic <- do_stn1%>%filter(DO_mgL<3&DO_mgL>0)

do_stn1_anoxic_summary <-do_stn1_anoxic%>%group_by(Date)%>%
  summarize(ave_DO=mean(DO_mgL,na.rm = TRUE))

do_stn1_hypoxic_summary <-do_stn1_hypoxic%>%group_by(Date)%>%
  summarize(ave_DO=mean(DO_mgL,na.rm = TRUE))

#
WL_df_1 <- read.csv(here::here("data_cleaned/WL_01_cleaned.csv"))%>%select(DateTime,WL_m,Q_m3s)
WL_df_1$DateTime <- as.POSIXct(WL_df_1$DateTime, format="%Y-%m-%d %H:%M",tz="UTC")

test <- WL_df_1$DateTime[duplicated(WL_df_1$DateTime)]
test2 <- as.data.frame(test)


DO_df <- inner_join(do_stn1%>%drop_na(DateTime),WL_df_1%>%drop_na(DateTime))

p <- ggplot(DO_df,aes(x=WL_m,y=DO_mgL)) + geom_point() +
...
```{r write out df}
####i am missing the data fram read out for all station data. so this is just the last bit of code 
...
all_stn_remove_4 <- all_stn %>%filter(DateTime!=as.POSIXct("2021-07-05 11:15:00",tz="UTC"))%>%select(DateTime,CO2_ppm_04,Q_m3s_04,WaterTemp_c_04)%>%filter(DateTime<as.POSIXct("2021-10-20 12:30:00",tz="UTC") | DateTime>as.POSIXct("2021-12-30 16:30:00",tz="UTC"))%>%filter(DateTime<as.POSIXct("2022-03-17 12:30:00",tz="UTC") | DateTime>as.POSIXct("2022-06-16 00:00:00",tz="UTC"))%>%filter(DateTime>"2021-06-06 00:00:00")%>%filter(DateTime!="2021-06-18 11:00:00
")

all_stn_remove_3 <- all_stn%>%filter(DateTime>"2021-07-06 00:00:00"&CO2_ppm_03>2500)%>%select(DateTime,CO2_ppm_03,Q_m3s_03,WaterTemp_c_03)


all_stn <- all_stn%>%select(DateTime,month_number,month,Q_m3s_01,CO2_ppm_01,WaterTemp_c_01,Q_m3s_02,CO2_ppm_02,WaterTemp_c_02)

all_stn2 <- full_join(all_stn,all_stn_remove_3,by="DateTime")
all_stn3 <- full_join(all_stn2,all_stn_remove_4,by="DateTime")

test <- all_stn3$DateTime[duplicated(all_stn$DateTime)]
test2 <- as.data.frame(test)

#write.csv(all_stn3,here::here("Whitmore_etal_Biogeochemistry/allstation_data_2.csv"))



#c6 data
C6_Q_df_storm_2 <- C6_Q_df_storm%>%#filter(is.na(DOC))
  select(DateTime,time_elapsed,Depth_m,CDOM_ppb,Q_m3s,CO2_ppm_01,DOC)%>%
  rename(pCO2_ppm=CO2_ppm_01)
C6_Q_df_storm_2$storm <- "storm"

C6_Q_df_DOC <- C6_Q_df%>%drop_na(DOC)%>%filter(DOC<4)%>%
  select(DateTime,Depth_m,CDOM_ppb,Q_m3s,CO2_ppm_01,DOC)%>%
  rename(pCO2_ppm=CO2_ppm_01)
C6_Q_df_DOC$time_elapsed <- NA
C6_Q_df_DOC$storm <- NA

C6_Q_df_2 <- rbind(C6_Q_df_DOC,C6_Q_df_storm_2)  
C6_Q_df_2$calibrate_DOC <- NA
C6_Q_df_2$calibrate_DOC <- ifelse(C6_Q_df_2$DOC>1,"calibrate",NA)

#write.csv(C6_Q_df_2,here::here("Whitmore_etal_Biogeochemistry/CDOM_data.csv"))

##DO data frame for sup fig 2 and depature plot
Stn01 <- read.csv(here::here("MergedFiles/Stn01_May7.csv"))
Stn01$Station.x <- NULL
Stn01$Station.y <- NULL
Stn01$X <- NULL
Stn01$Total_kpa <- NULL
Stn01 <- Stn01%>%rename(CO2_ppm=CO2_ppm_adjusted)
Stn01 <- Stn01%>%dplyr::select(DateTime,Date,AirPres_kpa, AirTemp_c,WLTemp_c,DOTemp_c,WL_m,WLTemp_c,Q_m3s,DO_mgL,DO_umolperL,DO_departure,CO2_ppm,CO2_umol.L,CO2_departure)%>%rename(DO_umol.L=DO_umolperL)
#write.csv(Stn01,here::here("Whitmore_etal_Biogeochemistry/DO_data_stn01.csv"))

#
departure_metrics <- read.csv(here::here("Departure_files/departure_metrics_may6.csv"))
#write.csv(departure_metrics,here::here("Whitmore_etal_Biogeochemistry/departure_metrics_stn01.csv"))
...
#write.csv(df_stn1_metab,here::here("Whitmore_etal_Biogeochemistry/metabolim_predictions_stn01.csv"))

#metaboilsm station 5
stn05 <- read.csv(here::here("data_cleaned/WL_05_cleaned.csv"))
stn05$DateTime <- as.POSIXct(stn05$DateTime, format="%Y-%m-%d %H:%M",tz="UTC")
stn05$date <- as.Date(stn05$DateTime,format="%Y-%m-%d")
stn05_summary<- stn05 %>%group_by(date)%>%
...
df_stn5_metab$GPP_97.5pct_CO2 <- (df_stn5_metab$GPP_97.5pct*44/32)
df_stn5_metab$ER_mean_CO2 <- (df_stn5_metab$ER_mean*44/32)
df_stn5_metab$ER_97.5pct_CO2 <- (df_stn5_metab$ER_97.5pct*44/32)
...
#remove high flows
df_stn5_metab <- df_stn5_metab%>%filter(discharge_stn05 < .1)
df_stn5_metab$X <- NULL
#write.csv(df_stn5_metab,here::here("Whitmore_etal_Biogeochemistry/metabolim_predictions_stn05.csv"),row.names=FALSE)
...
DOC_df <- rbind(DOC_2022,DOC_2021)

DOC_df$DateTime <- as.POSIXct(paste(DOC_df$Date, DOC_df$Time), format="%m/%d/%y %H:%M", tz = "UTC")
DOC_df$DateTime <- round_date(DOC_df$DateTime,unit="15 minutes")

#formating 2021 DOC data
DOC_df$Distance_from_peatland <- 
  ifelse(DOC_df$Station=="Stn01",5,
         ifelse(DOC_df$Station=="Stn02",56,
                ifelse(DOC_df$Station=="Stn03",288,
                       ifelse(DOC_df$Station=="Stn04",454,
                              ifelse(DOC_df$Station=="Stn05",0+1,52+1)))))
DOC_df$WS <- ifelse(DOC_df$Station=="Stn05"|DOC_df$Station=="Stn06","Colmillo","Gavilan")

#write.csv(DOC_df,here::here("Whitmore_etal_Biogeochemistry/DOC_df.csv"),row.names=FALSE)
...
CH4_df_summary <- CH4_df%>%group_by(Site,Site2,DateTime)%>%
  summarise(
    ...
    DO_mgL = mean(DO_mgL,na.rm=TRUE),
    Q_m3s = mean(Q_m3s),
    pCH4_uatm = mean(pCH4_ppmv),
    pCH4_ppm = mean(pCH4_ppmv),
    CH4_umol.L = mean(CH4_umol.L)
  )
...
#write.csv(CH4_plot_df,here::here("Whitmore_etal_Biogeochemistry/methane_df.csv"),row.names=FALSE)
