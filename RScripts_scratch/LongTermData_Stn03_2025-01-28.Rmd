...
WL_03$DateTime_1 <- as.POSIXct(WL_03$DateTime,format="%Y-%m-%d %H:%M",tz="UTC")
WL_03$DateTime <- ifelse(is.na(WL_03$DateTime_1)==TRUE,paste(WL_03$DateTime,"00:00:00",sep=" "),WL_03$DateTime )
WL_03$DateTime <- as.POSIXct(WL_03$DateTime,format="%Y-%m-%d %H:%M",tz="UTC")
WL_03$DateTime_1 <- NULL
WL_03 <- unique(WL_03)
WL_03 <- WL_03%>%drop_na(WLPres_kpa)%>%filter(DateTime!=as.POSIXct("2023-02-18 11:15:00
",tz='UTC'))


Baro <- read.csv(here::here("data_cleaned/Baro_cleaned.csv"))
Baro$DateTime <- as.POSIXct(Baro$DateTime, format="%Y-%m-%d %H:%M:%S",tz="UTC")
Baro <- Baro%>%filter(DateTime!=as.POSIXct("2021-06-18 11:00:00",tz='UTC'))%>%
  filter(DateTime!=as.POSIXct("2022-07-12 10:15:00",tz='UTC'))%>%
...
#test for duplicates
test <- CO2_03$DateTime[duplicated(CO2_03$DateTime)]
test2 <- as.data.frame(test)

##check data
p1 <- plot_ly(CO2_03, x = ~DateTime, y = ~ppm, type = 'scatter', mode = 'markers') 
...
ggplot(data = Stn03, aes(DateTime, adjusted_ppm)) +
  geom_point(color = "steelblue") +
  labs(#title = "CO2  stations",
       y = "CO2 ppm", x = "") 
...
)%>%rename(WLTemp_03_c=WLTemp_c,CO2_ppm = ppm,CO2_ppm_adjusted = adjusted_ppm,Total_hpa=Total_hPa)

Stn03$max_detect_ppm <- 10000

#write.csv(Stn03, here::here("data_cleaned/CO2_03_max10000_cleaned.csv"),row.names = FALSE)
...
##check data
p1 <- plot_ly(LUXabajo_03_Lux_data, x = ~DateTime, y = ~Lux, type = 'scatter', mode = 'lines') 


p2 <- plot_ly(LUXarriba_03_Lux_data, x = ~DateTime, y = ~Lux, type = 'scatter', mode = 'lines') 
...
LUXabajo_03_Lux_data <- LUXabajo_03_Lux_data%>%
  select(Station,DateTime,Lux,Temp_C)
#write.csv(LUXabajo_03_Lux_data, here::here("data_cleaned/LUX_underwater_03_cleaned.csv"),row.names = FALSE)

LUXarriba_03_Lux_data <- LUXarriba_03_Lux_data%>%
  select(Station,DateTime,Lux,Temp_C)
#write.csv(LUXarriba_03_Lux_data, here::here("data_cleaned/LUX_abovewater_03_cleaned.csv"),row.names = FALSE)
