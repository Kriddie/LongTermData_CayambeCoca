...
DO_06$DateTime <- as.POSIXct(DO_06$DateTime,format="%Y-%m-%d %H:%M",tz="UTC")
#rbind
Stn06 <- full_join(WL_06,DO_06, by=c("DateTime","Station"))

#the first DO measurement is 2021-06-11 13:00:00, so filter for after that
Stn06 <- Stn06%>%filter(DateTime > as.POSIXct("2021-06-11 13:00:00",tz="UTC"))
...
# locat time is GMT-5
Stn06$DateTime_UTC <- Stn06$DateTime + hours(5)
Stn06$solar.time <- convert_UTC_to_solartime(Stn06$DateTime_UTC,-78.200147, time.type = c("apparent solar", "mean solar"))

#max light based on la virgin in W/m2
#convert to umol/m2/s: *4.57
#convert to PAR *.455
Stn06$light <- calc_light(Stn06$solar.time,0.327992,-78.200147,max.PAR = u(1400*4.57*0.455, "umol m^-2 s^-1"),
...
Stn06$Q_m3s_unitted <- unitted(Stn06$Q_m3s, units = "m^3 s^-1")
...
Stn06$depth <- calc_depth(Stn06$Q_m3s_unitted, c = u(0.6383832, "m"), f = u(0.3108922, ""))
...
Stn06 <- Stn06%>%drop_na(solar.time)
first<-min(Stn06$solar.time)
last<-max(Stn06$solar.time)
solar.timeREF<-seq(first, last, by="15 min")

Stn06$ref<-match.closest(as.numeric(Stn06$solar.time), as.numeric(solar.timeREF), tolerance = 100, nomatch = NA)
df<-data.frame(ref2=seq(1:length(solar.timeREF)), solar.timeREF=solar.timeREF)

Dat2<-Stn06[match(df$ref2, Stn06$ref),]
...
#Write out
#write.csv(Dat2,here::here("metabolizer_dataframe/stn06_df_feb18.csv"),row.names = FALSE)
