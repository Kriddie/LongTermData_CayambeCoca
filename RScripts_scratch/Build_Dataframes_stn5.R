...
#water level
WL_06 <- read.csv(here::here("data_cleaned/WL_06_cleaned.csv"))%>%drop_na(Q_m3s)%>%select(!Station)
WL_06$DateTime <- as.POSIXct(WL_06$DateTime,format="%Y-%m-%d %H:%M",tz="UTC")
#DO
DO_05 <- read.csv(here::here("data_cleaned/DO_05_cleaned.csv"))
DO_05$DateTime <- as.POSIXct(DO_05$DateTime,format="%Y-%m-%d %H:%M",tz="UTC")
#rbind
Stn05 <- right_join(WL_06,DO_05, by=c("DateTime"))

#the first DO measurement is 2021-06-19 01:15:00, so filter for after that
Stn05 <- Stn05%>%filter(DateTime > as.POSIXct("2021-06-19 01:15:00",tz="UTC"))
#the last DO measurement is 2021-10-20 08:45:00
Stn05 <- Stn05%>%filter(DateTime < as.POSIXct("2021-10-20 08:45:00",tz="UTC"))

#calc DO sat using stream metabolizer funtion
Stn05$DO_sat <- calc_DO_sat(temp=u(Stn05$DOTemp_c,"degC"), press=u(Stn05$AirPres_kpa*10,"mb"), sal=u(0,"PSU")) # units are checked

# convert to solar.time
# locat time is GMT-5
Stn05$DateTime_UTC <- Stn05$DateTime + hours(5)
Stn05$solar.time <- convert_UTC_to_solartime(Stn05$DateTime_UTC,-78.200147, time.type = c("apparent solar", "mean solar"))

#max light based on la virgin in W/m2
#convert to umol/m2/s: *4.57
#convert to PAR *.455
Stn05$light <- calc_light(Stn05$solar.time,0.327992,-78.200147,max.PAR = u(1400*4.57*0.455, "umol m^-2 s^-1"),
...
Stn05$Q_m3s_unitted <- unitted(Stn05$Q_m3s, units = "m^3 s^-1")
...
Stn05$depth <- calc_depth(Stn05$Q_m3s_unitted, c = u(0.6383832, "m"), f = u(0.3108922, ""))
...
Stn05 <- Stn05%>%drop_na(solar.time)
first<-min(Stn05$solar.time)
last<-max(Stn05$solar.time)
solar.timeREF<-seq(first, last, by="15 min")

Stn05$ref<-match.closest(as.numeric(Stn05$solar.time), as.numeric(solar.timeREF), tolerance = 100, nomatch = NA)
df<-data.frame(ref2=seq(1:length(solar.timeREF)), solar.timeREF=solar.timeREF)

Dat2<-Stn05[match(df$ref2, Stn05$ref),]
