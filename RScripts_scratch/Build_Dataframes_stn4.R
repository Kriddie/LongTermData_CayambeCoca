...
WL_04 <- WL_04%>%filter(AirTemp_c!=21.862)
#DO
...
#the last DO measurment is 2023-04-01 11:30:00 
...
# locat time is GMT-5
Stn04$DateTime_UTC <- Stn04$DateTime + hours(5)
Stn04$solar.time <- convert_UTC_to_solartime(Stn04$DateTime_UTC,-78.200147, time.type = c("apparent solar", "mean solar"))
...
Stn04$Q_m3s_unitted <- unitted(Stn04$Q_m3s, units = "m^3 s^-1")
...
Stn04$depth <- calc_depth(Stn04$Q_m3s_unitted, c = u(0.6383832, "m"), f = u(0.3108922, ""))


#change names
Stn04 <- Stn04%>%select(solar.time,DO_mgL,DO_sat,depth,WLTemp_c,light,Q_m3s)
colnames(Stn04) <- c("solar.time","DO.obs","DO.sat","depth","temp.water","light","discharge")
...
Stn04 <- Stn04%>%drop_na(solar.time)
first<-min(Stn04$solar.time)
last<-max(Stn04$solar.time)
solar.timeREF<-seq(first, last, by="15 min")

Stn04$ref<-match.closest(as.numeric(Stn04$solar.time), as.numeric(solar.timeREF), tolerance = 100, nomatch = NA)
df<-data.frame(ref2=seq(1:length(solar.timeREF)), solar.timeREF=solar.timeREF)

Dat2<-Stn04[match(df$ref2, Stn04$ref),]
