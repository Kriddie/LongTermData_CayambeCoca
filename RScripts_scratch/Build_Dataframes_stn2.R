...
#read in light data
LUX_02 <- read.csv(here::here("data_cleaned/LUX_abovewater_02_cleaned.csv"))
LUX_02$DateTime <- as.POSIXct(LUX_02$DateTime,format="%Y-%m-%d %H:%M",tz="UTC")

#rbind
Stn02 <- full_join(WL_02,DO_02, by=c("DateTime","Station"))
Stn02 <- full_join(Stn02,LUX_02, by=c("DateTime","Station"))
...
Stn02$DO_sat <- calc_DO_sat(temp=u(Stn02$DOTemp_c,"degC"), press=u(Stn02$AirPres_kpa*10,"mb"), sal=u(0,"PSU")) # units are checked
...
Stn02$light <- Stn02$Lux
Stn02 <- Stn02%>%drop_na(light)
...
#change names
Stn02 <- Stn02%>%select(solar.time,DO_mgL,DO_sat,depth,DOTemp_c,light,Q_m3s)
...
#Write out
#write.csv(Dat2,here::here("metabolizer_dataframe/stn02_df_fieldLUX_march9.csv"),row.names = FALSE)
